#include "BST_MIP_MH.h"

Define_Module ( BST_MIP_MH );

/******************************************************************************/

void BST_MIP_MH::startup()
{
 ...
 */
void BST_MIP_MH::fromNetworkLayer ( ApplicationPacket * genericPacket, const char *source, double rssi, double lqi )
{
...
void BST_MIP_MH::timerFiredCallback ( int index )
{
	switch ( index ) {
	...
	case FIND_PATH: {
		double	minCost, totalCost, totalMinCost, travelDistance;
		int	i, j, k, l, m, n, s, v, currentNode, minNode, nodesInPath, prevNodesInPath, intermediateNodes, searchNode, nextNode, checked, inCurrentSet, currentSet[MAX_NUM_OF_SENSORS];
		bool	usedVec[MAX_NUM_OF_SENSORS];

		for ( s = savedS; s < numberOfSensors; s++ ) {
			/*
			 * Find every node that has the sink (node 0) as a parent in the MST.
			 * Starting from every such node we find all other nodes that belong to the same branch.
			 * Finally, we run the IEMF single itinerary algorithm to determine the order of nodes
			 * that the mobile agent has to follow for that specific branch.
			 */
			if ( PrevVect[s] == 0 ) {
				currentSet[0] = s;
				inCurrentSet = 1;
				checked = 0;
				while ( checked < inCurrentSet ) {
					for ( n = 0; n < numberOfSensors; n++ ) {
						if ( PrevVect[n] == currentSet[checked] ) {
							currentSet[inCurrentSet] = n;
							inCurrentSet++;
						}
					}
					checked++;
				...
				trace() << "In current set:";
				for ( n = 0; n < inCurrentSet; n++) {
					trace() << "  " << currentSet[n];
				}
				 ...
				 */
				iterations = inCurrentSet / 5;	// 20% of nodes in path.
				for ( m = 0; ( m < iterations ) && ( m < inCurrentSet ); m++ ) {
					totalMinCost = INFINITY;
					for ( i = 0; i < inCurrentSet; i++ ) {
						if ( ( Info[currentSet[i]][currentSet[i]].nodeID != NO_ID ) && ( Info[currentSet[i]][currentSet[i]].used == false ) ) {
							searchNode = currentSet[i];
							minNode    = currentSet[i];	// Handles case where only one sensor is in the surrent set.
#if defined(LOG_ENABLED)
							...
							for ( j = 0; j < inCurrentSet - m - 1; j++ ) {
								minCost = INFINITY;
								for ( k = 0; k < inCurrentSet; k++ ) {
									if ( ( Info[searchNode][currentSet[k]].cost < minCost ) && ( usedVec[currentSet[k]] == false ) ) {
										minCost = Info[searchNode][currentSet[k]].cost;
										minNode = currentSet[k];
									}
								}
#if defined(LOG_ENABLED)
...
#if defined(LOG_ENABLED)
							trace() << "  Total cost for second node " << currentSet[i] << " is " << totalCost;
#endif
							if ( totalCost < totalMinCost ) {
								totalMinCost = totalCost;
								nextNode = currentSet[i];
							}
						}
					}
				 ...
				 */
				for ( i = 0; i < inCurrentSet - m; i++ ) {
					minCost = INFINITY;
					for ( j = 0; j < inCurrentSet; j++ ) {
						if ( ( Info[currentNode][currentSet[j]].cost < minCost ) && ( Info[currentSet[j]][currentSet[j]].used == false ) ) {
							minCost = Info[currentNode][currentSet[j]].cost;
							minNode = currentSet[j];
						}
					}
...
//			}
			...
			if ( createMAPI == true ) {
				MAPIfd = fopen ( "BST_MIP_MH.mapi", "w" );
			...
			trace() << "Calculating edges that belong to the MST.";
			Prim( 0, PrevVect );

			for ( i = 0; i < numberOfSensors; i++ ) {
				trace() << "  From " << PrevVect[i] << " to " << i;
			}
...
void BST_MIP_MH::handleSensorReading ( SensorReadingMessage * sensorMsg )
{
...
void BST_MIP_MH::finishSpecific()
{
...
void BST_MIP_MH::DijkstraDistance ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
...
void BST_MIP_MH::DijkstraPower ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
	set<pair<double, int> > vertex_queue;
...
void BST_MIP_MH::Prim ( int source, vector<int> &PrevVect )
{
	list<int> notInMST;
	list<int> inMST;
	...
	inMST.clear();
	inMST.push_back ( source );
	for ( u = 0; u < numberOfSensors; u++ ) {
		if ( ( Info[u][u].nodeID != NO_ID ) && ( u != source ) ) {
			notInMST.push_back ( u );
		}
	}

	while ( !notInMST.empty() ) {
		min = INFINITY;
		for ( list<int>::iterator it_in = inMST.begin(); it_in != inMST.end(); it_in++ ) {
#if defined(LOG_ENABLED)
			trace() << "  Checking for node " << *it_in << " that is in MST";
#endif
			for ( list<int>::iterator it_out = notInMST.begin(); it_out != notInMST.end(); it_out++ ) {
				cost = a * Info[*it_in][*it_out].cost + ( 1 - a ) * ( Info[0][*it_in].cost + Info[0][*it_out].cost );
#if defined(LOG_ENABLED)
				trace() << "    Checking for node " << *it_out << " that is not in MST. a = " << a << ", cost = " << cost;
#endif
				if ( cost < min ) {
					min = cost;
					PrevVect[*it_out] = *it_in;
					u = *it_out;
				}
			}
		}

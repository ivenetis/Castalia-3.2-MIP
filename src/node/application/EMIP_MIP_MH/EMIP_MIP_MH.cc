#include "EMIP_MIP_MH.h"

Define_Module ( EMIP_MIP_MH );

/******************************************************************************/

void EMIP_MIP_MH::startup()
{
 ...
 */
void EMIP_MIP_MH::fromNetworkLayer ( ApplicationPacket * genericPacket, const char *source, double rssi, double lqi )
{
...
void EMIP_MIP_MH::timerFiredCallback ( int index )
{
	switch ( index ) {
	...
	case FIND_PATH: {
		double	minCost, totalCost, totalMinCost, Gmax, slope, intercept, distance, minDistance, theta, polarAngleVCL, dxStart, dyStart, dxEnd, dyEnd, dxNode, dyNode, travelDistance;
		...
		theta = 2 * M_PI / ZPE.size();
		while ( !ZPE.empty() ) {
			/*
			 * If there is only one node left as a starting point of an itinerary,
			 * add all remaining nodes to the path of that node.
			 *
			 * Otherwise, apply the normal EMIP-MIP algorithm.
			 */
			if ( ZPE.size() == 1) {
				it_i = ZPE.begin();
				nextNode = *it_i;
				...
				/*
				 * Find the line that connects the VCL and the sink.
				 */
				slope = ( Info[VCL][VCL].yCoord - Info[0][0].yCoord ) / ( Info[VCL][VCL].xCoord - Info[0][0].xCoord );
				intercept = Info[0][0].yCoord - slope * Info[0][0].xCoord;

				/*
				 * Find the distance of each node in ZPE from the line that connects the sink and the VCL found.
				 * Keep the node that has the minimum distance from that line. This node will be used as the
				 * first node for the next itinerary.
				 */
#if defined(LOG_ENABLED)
				trace() << "Finding distance of each node in ZPE from line (VCL, Sink):";
#endif
				minDistance = INFINITY;
				for ( it_i = ZPE.begin(); it_i != ZPE.end(); it_i++ ) {
					distance = abs ( ( slope * Info[*it_i][*it_i].xCoord - Info[*it_i][*it_i].yCoord + intercept ) ) / ( sqrt ( slope * slope + 1.0 ) );
#if defined(LOG_ENABLED)
					trace() << "  Distance for " << *it_i << " = " << distance;
#endif
				...
				trace() << "nextNode found is " << nextNode;

				/*
				 * This node will not be used again to find an itinerary.
				 */
				ZPE.erase( nextNode );

				/*
				 * Move the start of the axes to the position of 'nextNode' and find
				 * the angle of the VCL according to the new axes.
				 */
				polarAngleVCL = atan ( ( Info[VCL][VCL].yCoord - Info[nextNode][nextNode].yCoord ) / ( Info[VCL][VCL].xCoord - Info[nextNode][nextNode].xCoord ) );
#if defined(LOG_ENABLED)
				trace() << "theta         = " << theta;
				trace() << "polarAngleVCL = " << polarAngleVCL;
				trace() << "startAngle    = " << polarAngleVCL - theta / 2.0;
				trace() << "endAngle      = " << polarAngleVCL + theta / 2.0;
#endif

				/*
				 * The sector of interest in which we have to look for nodes to add to this itinerary has a center
				 * at 'nextNode' and its angle is from ( polarAngleVCL - theta / 2.0 ) to ( polarAngleVCL + theta / 2.0 ).
				 *
				 * In order to determine whether any remaining node lies in this sector,
				 * we find two vectors with a unit length and the following characteristics:
				 * - The first has the same direction as the line passing from 'nextNode' and direction 'startAngle'
				 * - The second has the same direction as the line passing from 'nextNode' and direction 'endAngle'
				 *
				 * Using these two vectors, it is very easy to determine whether any other node lies within the sector
				 * defined by these vectors, as described here:
				 *
				 * http://stackoverflow.com/questions/13652518/efficiently-find-points-inside-a-circle-sector
				 */
				dxStart = cos( polarAngleVCL - theta / 2.0 );
				dyStart = sin( polarAngleVCL - theta / 2.0 );
				dxEnd   = cos( polarAngleVCL + theta / 2.0 );
				dyEnd   = sin( polarAngleVCL + theta / 2.0 );
#if defined(LOG_ENABLED)
				trace() << "(" << dxStart << ", " << dyStart << "), (" << dxEnd << ", " << dyEnd << ")";
#endif
				...
				while ( it_i != it_end ) {
					...
					it_i++;
					/*
					 * Check if the current node is in the sector defined by the lines above.
					 */
					dxNode = Info[*it_prev][*it_prev].xCoord - Info[nextNode][nextNode].xCoord;
					dyNode = Info[*it_prev][*it_prev].yCoord - Info[nextNode][nextNode].yCoord;
					if ( ( dxStart * dyNode - dxNode * dyStart > 0.0 ) && ( dxEnd * dyNode - dxNode * dyEnd < 0.0 ) ) {
						Vleft.erase( *it_prev );
						Vgroup.insert( *it_prev );
					}
				}
			...
			/*
			 * Due to its definition, we know that all nodes in ZPE are within transmission range of the sink.
			 */
			 ...
			 */
			iterations = nodesInPath / 5;	// 20% of nodes in path.
...
//			}
			...
			if ( createMAPI == true ) {
				MAPIfd = fopen ( "EMIP_MIP_MH.mapi", "w" );
			...
			if ( ZPE.empty() ) {
				trace() << "Fatal error: No node could be added to ZPE.";
				exit(0);
			}

			trace() << "Nodes included in ZPE:";
			for ( set<int>::iterator it = ZPE.begin(); it != ZPE.end(); it++ ) {
				trace() << "  " << (*it);
			}
...
void EMIP_MIP_MH::handleSensorReading ( SensorReadingMessage * sensorMsg )
{
...
void EMIP_MIP_MH::finishSpecific()
{
...
void EMIP_MIP_MH::DijkstraDistance ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
...
void EMIP_MIP_MH::DijkstraPower ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
	set<pair<double, int> > vertex_queue;
